...
module.exports = new Object({
    ...
    async execute(client, message) {
        ...
        if (!message.member.voice.channel) {
            await client.util.oops(message.channel, 'You are not connected to a voice channel to queue songs.', color);
        ...
        if (!message.member.voice.channel.permissionsFor(client.user).has([PermissionFlagsBits.Connect, PermissionFlagsBits.Speak])) {
            await client.util.oops(message.channel, `I don't have enough permission to connect/speak in ${message.member.voice.channel}`);
            if (message) await message.delete().catch(() => { });
            return;
        }

        if (message.guild.members.me.voice.channel && message.guild.members.me.voice.channelId !== message.member.voice.channelId) {
            await client.util.oops(message.channel, `You are not connected to <#${message.guild.members.me.voice.channelId}> to queue songs`, color);
            if (message) await message.delete().catch(() => { });
            return;
        }

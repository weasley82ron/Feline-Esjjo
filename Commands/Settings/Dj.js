        ...
        if (["add", "a", "set", "s"].includes(args[0])) {
            ...
            if (!data) {
                ...
                if (!data.mode)
                    return await client.util.oops(
                        message.channel,
                        `Dj mode is currently disabled, please enable to use this sub command.`,
                        color,
                    );
            ...
            if (!data)
                return await client.util.oops(
                    message.channel,
                    `Don't have any dj role setup to use this sub command.`,
                    color,
                );
            ...
            return await client.util.msgReply(
                message,
                `Successfully deleted the dj role(s) setup.`,
                color,
            );
            ...
            if (!data.roles.length || data.roles.length <= 0)
                return await client.util.oops(
                    message.channel,
                    `Don't have any dj role(s) added to remove.`,
                    color,
                );
            if (!args[1])
                return await client.util.oops(
                    message.channel,
                    `Please provide the role number in dj role(s) list.`,
                    color,
                );
            ...
            if (index <= 0)
                return await client.util.oops(
                    message.channel,
                    `Role number shouldn't be lower than or equal to 0.`,
                    color,
                );
            if (index > data.roles.length)
                return await client.util.oops(
                    message.channel,
                    `Role number shouldn't be higher than the dj role(s) length.`,
                    color,
                );
            ...
            return await client.util.msgReply(
                message,
                `Removed role number \`[ ${index} ]\` from the dj role(s).`,
                color,
            );
            ...
            if (!data.roles.length || data.roles.length <= 0)
                return await client.util.oops(
                    message.channel,
                    `Don't have any dj role(s) added!`,
                    color,
                );
            let roles = [];
            ...
            if (pages.length <= 1) {
                ...
                let pbut = client
                    .button()
                    .setCustomId("dj_roles_list_but_previous")
                    .setEmoji("⬅️")
                    .setStyle(client.config.button.grey);
                let sbut = client
                    .button()
                    .setCustomId("dj_roles_list_but_stop")
                    .setEmoji("<:stolen_emoji:1099215378195943425>")
                    .setStyle(client.config.button.grey);
                let nbut = client
                    .button()
                    .setCustomId("dj_roles_list_but_next")
                    .setEmoji("➡️")
                    .setStyle(client.config.button.grey);
                        ...
                        components: [
                            client
                                ...
                                .addComponents(
                                    pbut.setDisabled(
                                        true,
                                        sbut.setDisabled(true),
                                        nbut.setDisabled(true),
                                    ),
                                ),
                        ],
                    });

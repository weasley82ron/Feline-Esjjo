    ...
    async execute(client, message, args, prefix, color, dispatcher) {
        ...
        const collector = m.createMessageComponentCollector({
            filter: (b) => {
                ...
                else { b.reply({ ephemeral: true, content: `Only **${interaction.member.user.tag}** can use this button.` }); return false; };
            },
            max: 1,
            time: 60000,
            idle: 60000 / 2
        ...
        collector.on('collect', async (int) => {
            if (int.customId === 'addprevious') {
                ...
                if (!dispatcher.queue.previous) return await m.edit({ embeds: [client.embed().setDescription('There is no previous track played.').setColor(color)], components: [client.row().addComponents([but1, but2])] }).catch(() => { });
                if (m) return await m.edit({ embeds: [embed.setDescription(`Added - \`${dispatcher.queue.previous.title}\` to the queue.`)], components: [client.row().addComponents([but1, but2])] }).catch(() => { });
                ...
                dispatcher.skip()
                await client.util.update(dispatcher, client);
                if (m) return await m.edit({ embeds: [client.embed().setDescription(`Added - \`${dispatcher.queue.previous.title}\` to the queue.`).setColor(color)], components: [client.row().addComponents([but1, but2])] }).catch(() => { });
            }
        })
    }
})
    ...
    async execute(client, message, args, prefix, color, dispatcher) {
        if (args.length) {
            if (["reset", "r"].includes(args[0])) {
                ...
                return await client.util.msgReply(message, `Speed set to default \`[ ${dispatcher.speedAmount}x ]\``, color);
            } else {
                let value = parseInt(args[0]);
                if (isNaN(value)) return await client.util.oops(message.channel, `Please provide a valid number, 1, 2, or 3.`, color);

                if (value <= 0 || value > 3) return await client.util.oops(message.channel, `Please provide a valid number, 1, 2 or 3.`, color);

                dispatcher.setSpeed(value);
                return await client.util.msgReply(message, `Speed set to \`[ ${dispatcher.speedAmount}x ]\``, color);
            };
        } else {
            ...
            collector.on("collect", async (button) => {
                ...
                if (button.customId === "speed_button_1") {
                    ...
                    return await client.util.intReply(button, `Speed set to \`[ ${dispatcher.speedAmount}x ]\``, color);
                    ...
                    return await client.util.intReply(button, `Speed set to default \`[ ${dispatcher.speedAmount}x ]\``, color);
                } else return;
            });
        };
    },

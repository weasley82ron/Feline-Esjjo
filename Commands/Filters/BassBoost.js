    ...
    async execute(client, message, args, prefix, color, dispatcher) {
        if (args.length) {
            if (["none", "n"].includes(args[0])) {
                if (dispatcher.bassboostLevel === "none") return await client.util.oops(message.channel, `Bassboost level is already at \`[ ${dispatcher.bassboostLevel} ]\``, color);

                dispatcher.setBassboost("none");
                return await client.util.msgReply(message, `Bassboost level set to \`[ ${dispatcher.bassboostLevel} ]\``, color);
            } else if (["low", "l"].includes(args[0])) {
                if (dispatcher.bassboostLevel === "low") return await client.util.oops(message.channel, `Bassboost level is already at \`[ ${dispatcher.bassboostLevel} ]\``, color);

                dispatcher.setBassboost("low");
                return await client.util.msgReply(message, `Bassboost level set to \`[ ${dispatcher.bassboostLevel} ]\``, color);
            } else if (["medium", "m"].includes(args[0])) {
                if (dispatcher.bassboostLevel === "medium") return await client.util.oops(message.channel, `Bassboost level is already at \`[ ${dispatcher.bassboostLevel} ]\``, color);

                dispatcher.setBassboost("medium");
                return await client.util.msgReply(message, `Bassboost level set to \`[ ${dispatcher.bassboostLevel} ]\``, color);
            } else if (["high", "h"].includes(args[0])) {
                if (dispatcher.bassboostLevel === "high") return await client.util.oops(message.channel, `Bassboost level is already at \`[ ${dispatcher.bassboostLevel} ]\``, color);

                dispatcher.setBassboost("high");
                return await client.util.msgReply(message, `Bassboost level set to \`[ ${dispatcher.bassboostLevel} ]\``, color);
            } else return await client.util.invalidArgs("bassboost", message, `Please provide a valid bassboost level, \`none, low, medium, high\``, client);
        } else {
            ...
            collector.on("collect", async (button) => {
                ...
                if (!dispatcher) return await client.util.intReply(button, `Nothing is playing right now to set bassboost level.`, color);
                if (!dispatcher.queue) return await client.util.intReply(button, `Nothing is playing right now to set bassboost level.`, color);
                if (!dispatcher.queue.current) return await client.util.intReply(button, `Nothing is playing right now to set bassboost level.`, color);
                ...
                if (button.customId === "bb_button_none") {
                    ...
                    return await client.util.intReply(button, `Bassboost level set to \`[ ${dispatcher.bassboostLevel} ]\``, color);
